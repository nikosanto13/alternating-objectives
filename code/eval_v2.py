...
def eval_PGD(model_name: str,
  ...
  if dataset == 'CIFAR10':
    ...
    x_test, y_test = load_imagenet(n_examples=20,data_dir=IMAGENET_PATH)
    batch_size = 20
  ...
  for i in tqdm(range(num_of_batches)):
    ...
    incorrect = (y!=pred)
    xi, xadvi, perturb = x[incorrect], xadv[incorrect], (x-xadv)[incorrect]
    all_ims = torch.vstack((torch.vstack((xi[:8],xadvi[:8])),3*perturb[:8]))
    save_image(all_ims, 'images/img_stacked.png')
