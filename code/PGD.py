...
def PGD(model,x,y,
  ...
  model.eval()
  x_adv = x.clone()
  best_loss = -INF*torch.ones(x.size(0)).to(device)
  
  # N is the number of stages for each PGD variant. E.g. when we use a single loss during PGD, then N = 1 
  N = len(kwargs['losses']) if (loss_fn == Alternate or loss_fn == Linear) else 1 
  kwargs['T'] = T  
  ...
  for r in range(restarts):
    ...
    if init == 'random' or restarts > 1:
      random_noise = torch.FloatTensor(xn.shape).uniform_(-epsilon, epsilon).to(device)
    ...
    for t in range(T):
      ...
      alpha_t  = step_schedule(alpha,**kwargs2) if step_schedule else alpha  
...
def PGD_distances(model,x,y,
  ...
  for r in range(restarts):
    ...
    for t in range(T):
      ...
      xn = Variable(x.data + eta, requires_grad=True)
      xn = Variable(torch.clamp(xn, 0, 1.0), requires_grad=True)
      deltas[t] = xn.detach().clone() - xn_prev
