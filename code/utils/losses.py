...
def CW(x,y,**kwargs):
  ...
  u = torch.arange(px.shape[0])    

  return -(px[u, y] - px_sorted[:, -2] * ind - px_sorted[:, -1] * (1. - ind))
...
def Alternate(x,y,**kwargs):
  ...
  assert len(timesteps) == len(losses), 'Num of loss functions should be the same with num of timesteps where switching occurs (+ the timestep t=T)'
...
def Linear(x,y,**kwargs):
  ...
  '''
  assert len(kwargs['losses']) == 2, 'Currently, this loss function is only supported for two objectives'
  assert len(kwargs['losses']) == len(kwargs['coeffs']), 'Every loss functions should have a corresponding coefficient'
  ...
  if 'decay' in kwargs.keys():
    assert len(kwargs['decay']) == 2, 'Num of decaying timesteps is not equal to num of losses'
